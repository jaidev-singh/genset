      ...
      <div style={{
        ...
        {!role ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {ROLES.map(r => (
              <button
                ...
                style={{ padding: "14px 18px", borderRadius: 10, border: "1px solid #e2e8f0", background: "white", cursor: "pointer", textAlign: "left" }}
                onMouseEnter={e => { e.currentTarget.style.background = "#f8fafc"; e.currentTarget.style.borderColor = "#94a3b8" }}
                onMouseLeave={e => { e.currentTarget.style.background = "white"; e.currentTarget.style.borderColor = "#e2e8f0" }}
              >
                <div style={{ fontWeight: 700, fontSize: 14, color: "#1e293b" }}>{r.emoji} {r.label}</div>
          ...
          <form onSubmit={handleSubmit}>
            <button type="button" onClick={() => { setRole(null); setError(""); setPassword(""); setUsername("") }}
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <input value={username} onChange={e => setUsername(e.target.value)}
                required autoFocus autoCapitalize="none" autoCorrect="off" autoComplete="username"
                placeholder={role === "boss" ? "boss" : role === "admin" ? "admin" : "your username"}
            ...
            <button type="submit" disabled={loading}
              style={{ width: "100%", padding: "12px", background: loading ? "#93c5fd" : "#1a73e8", color: "white", border: "none", borderRadius: 8, fontWeight: 700, fontSize: 15, cursor: loading ? "default" : "pointer" }}>
